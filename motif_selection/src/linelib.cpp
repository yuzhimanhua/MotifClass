...
void line_node::init(char *file_name, int vector_dim)
{
	...
	if (_vec == NULL) { printf("Memory allocation failed (embedding)\n"); exit(1); }

	for (a = 0; a < node_size; a++)
		for (b = 0; b < vector_size; b++)
			_vec[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;

	kappa = (double *)malloc(node_size * sizeof(double));
	if (kappa == NULL) { printf("Memory allocation failed (kappa)\n"); exit(1); }
...
void line_node::output(char *file_name, char *kappa_file, int binary)
{
...
void line_trainer::train_sample(real alpha, real *_error_vec, double(*func_rand_num)(), unsigned long long &rand_index)
{
	...
	real kappa = node_u->kappa[u], kappa_update = 0.0, emb_lambda = 1.5, kappa_lambda = 0.5;
	for (int d = 0; d < neg_samples + 1; d++)
	{
		...
		error_vec += g * kappa * ((node_v->vec.row(target)));
		node_v->vec.row(target) += g * kappa * ((node_u->vec.row(u)));
	}
	node_u->vec.row(u) += error_vec;

	node_u->kappa[u] += kappa_lambda * kappa_update;
	if (node_u->kappa[u] < 0)
		node_u->kappa[u] = 0;
